...
export default function CartBadge() {
  ...
  return (
    <a href="/checkout" className="relative flex items-center gap-2 text-floof-dark/70 hover:text-floof-pink transition-colors" aria-label={`Cart with ${itemCount} items`}>
      ...
      {m && !loading && itemCount > 0 && <span className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-floof-pink text-white text-xs font-bold flex items-center justify-center">{itemCount > 99 ? "99+" : itemCount}</span>}
    </a>
  )
}